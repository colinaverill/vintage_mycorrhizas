#Visualize neighbor effects on mortality from gam fits.
rm(list=ls())
source('paths.r')
library(mgcv)
library(MASS)
...
#n.sim <- 300
#par    <- coef(fit)[1:4]
#covariance <- vcov(fit)[1:4,1:4]
#draw from multivariate normal
#sim.mc <- mvrnorm(n.sim, par, covariance)
#am.am <- sim.mc[,1]
#am.em <- sim.mc[,1] + sim.mc[,3]
#em.am <- sim.mc[,1] + sim.mc[,2]
#em.em <- rowSums(sim.mc)
...
png('mortality_effects_ESA.png', width = 5.5, height = 3.5, units = 'in', res = 300)
...
limy <- c(min(c(y.am,y.em)), max(c(y.am,y.em)) * 1.2)
...
abline(v = 1.25, lwd=2)
#labels.----
mtext('Mortality Probability', side = 2, cex = 1.3, line = 2.5)
