#Redoing simulations with 50-50 split and 120yr spin up, spin dow.
rm(list=ls())
source('paths.r')
...
#set output path.----
output.path <- forest_sim_within.plot.myc120.path
...
for(i in 1:length(ndep.down.range)){
  ...
  down.nul[[i]] <- makeitwork(
    forest.sim(g.mod    = fits$n.feedback$G.mod, 
               ...
               env.cov = env.cov, 
               silent = T,
               n.plots = 1000, n.step = 44,
               step.switch = 24, switch.lev = ndep.down.range[i],
               n.cores = n.cores)
  )
